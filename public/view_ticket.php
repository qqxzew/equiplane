        ...
        <div class="mt-6 space-y-4 text-sm">
            ...
            <?php if ($ticket['status'] === 'closed'): ?>
                <div class="border-t border-gray-800 pt-4 mt-4 text-xs space-y-1">
                    <div><span class="text-gray-500">Hours spent:</span> <span
                                class="text-white font-mono"><?= $ticket['hours_spent'] ?> h</span></div>
                    <div><span class="text-gray-500">Parts cost:</span> <span
                                class="text-white font-mono"><?= $ticket['cost_of_parts'] ?> Kč</span></div>
                </div>
            <?php endif; ?>
                ...
                <div class="border border-red-500/20 bg-red-500/5 p-4 rounded-lg mt-4 space-y-3">
                    ...
                    <form action="view_ticket.php?id=<?= $ticketId ?>" method="POST" class="flex gap-2">
                        <button type="submit" name="admin_backup_decision" value="approve"
                                class="bg-green-600 hover:bg-green-500 text-white text-xs px-3 py-1.5 rounded transition font-medium">
                            Accept & Add Engineers
                        </button>
                        <button type="submit" name="admin_backup_decision" value="reject"
                                class="bg-gray-800 hover:bg-gray-700 text-gray-400 text-xs px-3 py-1.5 rounded transition font-medium">
                            Reject Request
                        </button>
                    </form>
                </div>
                    ...
                    <?php if ($ticket['status'] === 'new'): ?>
                        <form action="view_ticket.php?id=<?= $ticketId ?>" method="POST">
                            <button type="submit" name="ticket_action" value="start"
                                    class="bg-blue-600 hover:bg-blue-500 text-white text-xs px-4 py-2 rounded transition font-medium">
                                Start Work
                            </button>
                        </form>
                    <?php endif; ?>
                    ...
                    <?php if ($ticket['status'] === 'in_progress' || $ticket['status'] === 'backup_required'): ?>
                        <div class="space-y-4 max-w-xs">
                            ...
                            <?php if ($ticket['status'] === 'in_progress'): ?>
                                <form action="view_ticket.php?id=<?= $ticketId ?>" method="POST"
                                      class="border-b border-gray-800 pb-4 space-y-2">
                                    <label for="backup_reason" class="block text-xs text-gray-400 font-medium">Specify
                                        Backup Reason:</label>
                                    <input type="text" name="backup_reason" id="backup_reason" required
                                           placeholder="e.g. Need second person for heavy lifting"
                                    ...
                                    <button type="submit" name="ticket_action" value="backup"
                                            class="bg-red-600 hover:bg-red-500 text-white text-xs px-3 py-1.5 rounded transition font-medium">
                                        🚨 Request Backup
                                    </button>
                                </form>
                            <?php endif; ?>
                            ...
                            <form action="view_ticket.php?id=<?= $ticketId ?>" method="POST" class="space-y-3 pt-2">
                                ...
                                <div>
                                    <label for="hours_spent" class="block text-xs text-gray-500 mb-1">Hours
                                        spent:</label>
                                    <input type="number" step="0.1" name="hours_spent" id="hours_spent" required min="0"
                                           placeholder="e.g. 2.5"
                                           class="w-full bg-gray-950 border border-gray-800 rounded px-2.5 py-1.5 text-white text-xs focus:outline-none">
                                </div>

                                <div>
                                    <label for="cost_of_parts" class="block text-xs text-gray-500 mb-1">Parts cost
                                        (Kč):</label>
                                    <input type="number" step="1" name="cost_of_parts" id="cost_of_parts" required
                                           min="0" placeholder="e.g. 1500"
                                           class="w-full bg-gray-950 border border-gray-800 rounded px-2.5 py-1.5 text-white text-xs focus:outline-none">
                                </div>

                                <button type="submit" name="ticket_action" value="resolve"
                                        class="bg-green-600 hover:bg-green-500 text-white text-xs px-4 py-2 rounded transition font-medium">
                    ...
                    <form action="view_ticket.php?id=<?= $ticketId ?>" method="POST" class="space-y-2">
                        ...
                        <div class="flex gap-2">
                            <select name="engineer_id" id="engineer_id" required
                                    class="bg-gray-950 border border-gray-800 rounded px-3 py-1.5 text-white focus:outline-none text-xs">
                            ...
                            <button type="submit" name="assign_engineer"
                                    class="bg-orange-600 hover:bg-orange-500 text-white text-xs px-4 py-1.5 rounded transition">
                                Assign
                            </button>
                        </div>
                    </form>
            ...
            <div class="border-t border-gray-800 pt-4">
                ...
                <div class="space-y-1 text-xs font-mono text-gray-500">
                    ...
                    <?php foreach ($logs as $log): ?>
                        <div>[<?= date('d.m.Y H:i', strtotime($log['created_at'])) ?>
                            ] <?= htmlspecialchars($log['action_text'], ENT_QUOTES, 'UTF-8') ?></div>
                    <?php endforeach; ?>
                </div>
            </div>
        </div>

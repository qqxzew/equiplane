...
?>
    ...
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            ...
            <?php if ($role === 'Client'): ?>
                <a href="create_ticket.php"
                   class="bg-orange-600 hover:bg-orange-500 text-white text-sm font-medium py-2.5 px-5 rounded-lg transition shadow-lg shadow-orange-600/10">
                    + New Application
                </a>
            <?php endif; ?>
        </div>
        ...
        <form method="GET" action="tickets.php"
              class="bg-gray-900 p-4 rounded-xl border border-gray-800 flex flex-wrap gap-4 items-end text-xs">
            <div class="flex flex-col gap-1">
                <label for="status" class="text-gray-500 font-medium">Status</label>
                <select name="status" id="status"
                        class="bg-gray-950 border border-gray-800 rounded-md px-3 py-2 text-white focus:outline-none focus:border-orange-500/50">
            ...
            <div class="flex gap-2">
                <button type="submit"
                        class="bg-orange-600 hover:bg-orange-500 text-white px-5 py-2 rounded-md transition font-medium">
                    Apply Filters
                </button>
                <?php if (!empty($filterStatus) || !empty($filterPriority) || !empty($searchQuery)): ?>
                    <a href="tickets.php"
                       class="bg-gray-850 hover:bg-gray-800 text-gray-400 px-4 py-2 rounded-md transition flex items-center justify-center font-medium border border-gray-800">
                        Reset
                    </a>
                <?php endif; ?>
            </div>
                            ...
                            <tr class="hover:bg-gray-850/40 transition">
                                ...
                                <td class="py-4 px-6 text-right">
                                    <a href="view_ticket.php?id=<?= $ticket['id'] ?>"
                                       class="text-xs font-medium text-orange-500 hover:text-orange-400 transition">
                                        View Details →
                                    </a>
                                </td>
                            </tr>
